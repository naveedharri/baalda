...
- Big vaults sync in bulk: turning on sync for a large folder registers and uploads notes in batches instead of one connection per note, and joining a large team downloads the whole vault as a compressed stream with a progress count in the header. Interrupted runs resume where they stopped, and a file on disk is never overwritten by the download: if it differs, the two versions are merged the way a live edit is.
