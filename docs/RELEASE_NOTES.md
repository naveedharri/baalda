...
- Files anywhere in your vault, not just images pasted into notes, now sync between devices and teammates with their real names and folders; a shared folder shares the files in it, and the AI assistant can list and read attachment text it has access to. The Access page lists those files next to your notes, so you can set who reaches each one. Renaming a synced file does not yet propagate.
