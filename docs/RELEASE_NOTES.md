...
- Files anywhere in your vault, not just images pasted into notes, now sync between devices and teammates with their real names and folders; a shared folder shares the files in it, and the AI assistant can list and read attachment text it has access to. The Access page lists those files next to your notes, so you can set who reaches each one. Deleting or renaming a synced file now follows too: it stays deleted instead of coming back on the next sync, a renamed file keeps its sharing, and setting a file to Private now takes it off every device that can no longer read it — yours included — instead of leaving a readable copy behind.
